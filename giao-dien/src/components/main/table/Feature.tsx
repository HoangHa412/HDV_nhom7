...
import {CAS, feature} from "@/types";

const Features: FC<{ data1: feature; data2: feature }> = ({data1, data2}) => {
    ...
    return (
        <div className="">
            ...
            <div
                className={`shadow-xl overflow-hidden transition-all duration-1000 ${isOpen ? "max-h-screen opacity-100" : "max-h-0 opacity-0 hidden"}`}
            >
                <div
                    className="p-3 mt-2 border border-t-black rounded-8  max-h-[400px] overflow-y-auto"
                >
                    <table className="w-full text-left border-collapse">
                        <thead>
                        {renderRow("Bảo mật nâng cao", data1.bao_mat, data2.bao_mat)}
                        {renderRow(
                            "Tính năng đặc biệt",
                            data1.tinh_nang_dac_biet,
                            data2.tinh_nang_dac_biet
                        )}
                        {renderRow("Kháng nước, bụi", data1.ghi_am, data2.ghi_am)}
                        {renderRow("Xem phim", data1.xem_phim, data2.xem_phim)}
                        {renderRow("Nghe nhạc", data1.nghe_nhac, data2.nghe_nhac)}
                        </thead>
                    </table>
                </div>
            </div>
